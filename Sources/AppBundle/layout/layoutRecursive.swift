            ...
            case .window(let window):
                if window.windowId != currentlyManipulatedWithMouseWindowId {
                    ...
                    if window.isFullscreen && window == context.workspace.rootTilingContainer.mostRecentWindowRecursive {
                        ...
                        let prevAppliedRect = lastAppliedLayoutPhysicalRect
                        lastAppliedLayoutPhysicalRect = physicalRect
                        window.isFullscreen = false
                        // The AX write is skipped when the window is already known to be at this exact frame.
                        // External moves/resizes reset lastAppliedLayoutPhysicalRect (see invalidateAxFrameCaches)
                        if prevAppliedRect != physicalRect {
