...
AeroSpace has built-in window borders, so you don't need an external tool. When enabled, a
border is drawn around every visible window: the focused window uses `active-color`, all others
use `inactive-color`. Borders track focus/layout from AeroSpace's refresh loop, and track live
drags from WindowServer move/resize events (no separate process). Move events request notifications
for tracked windows and paint immediately so borders stay glued during a drag.

[source,toml]
----
window-borders.enabled =        true
# Colors accept both 0xAARRGGBB (JankyBorders style) and #RRGGBB / #RRGGBBAA (CSS)
window-borders.active-color =   '0xffe1e3e4'
window-borders.inactive-color = '0xff494d64'
window-borders.width =          4
window-borders.corner-radius =  10
----

NOTE: `corner-radius` is a single value you tune to taste. macOS provides no API to read a
window's actual corner radius (and on recent macOS the radius even varies by window type — windows
with toolbars are more rounded), so a border can't perfectly match every app automatically.
Standard windows are around `10`.

For the odd app whose rounding doesn't match the default, set a per-app override keyed by its bundle
id (find it with `aerospace list-apps`):

[source,toml]
----
window-borders.corner-radius = 10
window-borders.corner-radius-overrides = { "com.apple.Terminal" = 6, "com.mitchellh.ghostty" = 12 }
----

If you were using the external link:https://github.com/FelixKratz/JankyBorders[JankyBorders]
tool, remove its `exec-and-forget borders ...` line so the two don't fight over the same windows.
