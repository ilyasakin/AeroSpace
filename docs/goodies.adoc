...
use `inactive-color`. Borders are driven from AeroSpace's own layout loop (no separate tracking
process), so they track focus, moves, resizes and workspace switches for free.
